...
def setup_freecad():
    ...
    candidate_paths = [
        ...
        fr"C:\Users\{username}\AppData\Local\Programs\FreeCAD 1.0"
    ]
    ...
    if freecad_path is None:
        root = tk.Tk()
        root.withdraw()
        print("FreeCAD not found in default locations. Please select the FreeCAD install folder.")
        selected_path = filedialog.askdirectory(title="Select FreeCAD installation folder")
        if selected_path and os.path.exists(selected_path):
            freecad_path = selected_path
        else:
            raise RuntimeError("FreeCAD installation not found.")

    # Add FreeCAD paths
    sys.path.append(os.path.join(freecad_path, "bin"))
    sys.path.append(os.path.join(freecad_path, "lib"))

    # Ensure DLLs are in PATH
    os.environ["PATH"] = os.path.join(freecad_path, "bin") + os.pathsep + os.environ["PATH"]
    ...
    try:
        ...
        raise RuntimeError(f"Could not import FreeCAD modules. Check installation. Error: {e}")

    print(f"✅ FreeCAD loaded from: {freecad_path}")
    return freecad_path
...
class Roof:
    ...
    def __init__(self, 
                    ...
                    roof_corner_angle_threshold, roof_use_average_corners, roof_N_average_corner_points,
        ...
        self.roof_use_average_corners = roof_use_average_corners
        self.roof_N_average_corner_points = roof_N_average_corner_points
...
############################################################################################### ALTERED SECTION
        ## Extract midline points from matplotlib contour
        #CS = plt.contour(x, y, mid_field_masked, levels=[0], colors='green')
        ##plt.plot(concaveRing[0][:,0], concaveRing[0][:,1], 'r-')
        ##plt.scatter(pts_1[:,0], pts_1[:,1], c='blue', s=5)
#
        ## Grab points from the contour
        #midline_points = []
        #for collection in CS.collections:
        #    for path in collection.get_paths():
        #        verts = path.vertices
        #        midline_points.append(verts)
        ## midline_points is now a list of arrays (possibly multiple disconnected curves)
        ## If multiple curves exist, you can choose the longest one:
...
#
    ...
    def get_major_corners_and_directions_for_roofs(self):
        ...
        use_average_corners = self.roof_use_average_corners
        N_average_corner_points = self.roof_N_average_corner_points
        ...
        if use_average_corners == False:
            direction_vectors = np.array(self.get_corner_vectors(concaveRing[0],corner_indices))
        elif use_average_corners == True:
            direction_vectors = np.array(self.get_avg_corner_vectors(concaveRing[0], corner_indices,Navg=N_average_corner_points))
        else:
            raise ValueError("Set use_average_corners to True or False")
...
class Tap:
    ...
    def get_pcd(self):
        self.pcd = o3d.geomtry.PointCloud()
        self.pcd.points = o3d.utility.Vector3dVector(np.array(self.coordinates))
...
class Corner:
    ...
    def get_pcd(self):
        ...
        #self.pcd.points = o3d.utility.Vector3dVector(np.array(self.coordinates))
#
        #if self.major == np.True_: self.myColor = [1.,0,0]
        #else: self.myColor = [.4,.1,.1]
#
        #self.pcd.paint_uniform_color(self.myColor)
#
        #return self.pcd
...
class Building:
    ...
    def __init__(self,clad_file_path):

        # Replace the file 
        self.clad_file_path = clad_file_path.replace("\\","/")
        self.clad_folder_path = None
        self.shape_brep_file=None
        self.unscaled_mesh_stl_file = None
        self.metadata_file = None
        self.unscaled_taps_csv_file = None
        self.force_clad_extraction = False # If true, will compute the CLAD extraction even if files exist

        self.scale_factor = None
        self.unscaled_mesh = None
        self.unscaled_taps = None
        self.scaled_mesh = None
        self.scaled_mesh_trimesh = None
        self.scaled_taps = None

        self.force_total_pcd_recompute = False # If true, will recompute the total pcd
        self.mesh_pcd_generation_multiplier = 10000 # Default Value
        ...
        self.roof_xy_eps = 0.51                     # Epsilon value foe the hierarchical clustering of roof points in XY, should probably be just higher than the spacing oif the mesh lattice. Larger values may agglomerate roofs of the same z-value that are actually distinct, smaller values may omit roofs
        ...
        self.roof_concave_hull_length_threshold = 0.48 #length threshold for concave_hull for roof major corner detection. Probably doesn't need to change for a given spacing
        ...
        self.roof_use_average_corners = False        # If you want to use the average corner directions ratyher than the immediate corner directrions.
        self.roof_N_average_corner_points = 4        # Number of points uised in average corner direction calculation
        ...
        self.roof_max_distance_from_model=.46        # BEfore merging roof edge points, remove any points that are not near the roof (for whatever reason, usually weird point detection). THis is the threshold distance. THis probably shouldn't be a user parameter but I will need to tune it.
        self.roof_edge_maximum_filtering_distance=2. # This is the distance roof edge-points can be from each other before they are replaced by the average of the points. Higher values will mean many points get averaged together, lower valuers mean fewer points do. 0 means this doesn't happen at all. infinity means all roof edges are replaced by a single point ( for each roof)
        self.roof_distance_to_concave_hull_filtration=1.3  # THis will remove any points that are more than X away from the concave hull itself ( for roof edge points only -- prevents crazy tap placement for badly computed concave hulls)
        ...
        self.facade_epsilon = 0.26                     # Is ideally equal to or slightly higher than HALF the spacing of the points
        self.facade_concave_hull_length_threshold = .48              # Is ideally equal to or slightly lower than the spacing of the points
        ...
        self.facade_use_average_corners = False        # If you want to use the average corner directions ratyher than the immediate corner directrions.
        self.facade_N_average_corner_points = 4        # Number of points uised in average corner direction calculation
        ...
        #If you want to add manual taps
        self.facade_manual_taps = None



        # Begin initialization methods
        # Get the folder that the clad file is in, check if it is just the current folder then set the clad_folder Path to the current working directory
        self.clad_folder_path = self.set_clad_folder_working_directory()

        # Next we need to check the folder to see if the data has already been extracted and then extract it if it hasn't
        self.extract_clad_data(self.force_clad_extraction)

        # Next, we need to take the files extracted from the CLAD file and construct scaled versions of the tap and mesh files.
        self.scale_factor = self.get_scale_factor()

        # Next we need to compute the scaled mesh and scaled taps, or, if the files already exist, set the self.scaled_mesh and self.scaled_taps variables
        self.get_scaled_mesh_and_taps()
        ...
        self.total_pcd = self.get_total_pcd()

        # Now, we get the rooflines and the fillzed in Z-levels
        self.roof_lines = self.compute_and_identify_roofs_and_rooflines()

        # Fill in the gaps on the facade based on the parameter
        self.facade_z_levels=self.fill_z_gaps()

        if self.filter_whole_z_levels == True:
                self.facade_z_levels=self.filter_whole_zlevels()

        # Get the roof clusters
        self.roof_clusters = self.get_roof_clusters()

        # Next we need to initialize The Facade ZLevel objects
        self.ZLevels = self.initialize_ZLevels()

        # Next we need to initialize the Roof Objects
        self.Roofs = self.initialize_Roofs()
        ...
        self.PredictedTaps = self.collect_taps()

        self.PredictedTapPCD = self.get_predicted_tap_pcd()
    ...
    def initialize_Roofs(self):
        ...
        for i in range(len(self.roof_clusters)):
            roof_objects.append(Roof(self.roof_clusters[i], self.total_pcd,
                    ...
                    self.roof_corner_angle_threshold, self.roof_use_average_corners, self.roof_N_average_corner_points,
        ...
        if self.facade_double_tap_list is not None:
            ...
            for i in range(len(self.facade_z_levels)):
                new_z_level = ZLevel(self.facade_z_levels[i]-self.facade_zLevel_lower, self.total_pcd, 
                                             ...
                                             self.facade_nzFiltercutoff,self.facade_corner_angle_threshold, self.facade_use_average_corners, self.facade_N_average_corner_points,
    ...
    def set_clad_folder_working_directory(self):
        print(self.clad_file_path)
        if len(self.clad_file_path.split('/')) > 1:
            return '/'.join(self.clad_file_path.split('/')[:-1])+'/'
        else:
            return self.resource_path(os.getcwd()+'/')
        
    def extract_clad_data(self,force_clad_extraction = False):

        """
        Method to extract the tap data, shape file, mesh, and model MetaData From a CLAD File
         - The function will not perform the extraction if all of the files already exist in the path folder.
         - The function can be forced to redo the extraction if force_clad_extraction = True is passed to the function
         - This will not be the default because but can be done by a user.
        
         Arguments:
          - Self
          - force_clad_extraction (default: False), if set to True, will redo the CLAD extraction even if the files already exist.
        """

        extracted_file_paths = [os.path.join(self.clad_folder_path, "shape.brep"),
                                os.path.join(self.clad_folder_path, "mesh.stl"),
                                os.path.join(self.clad_folder_path, "model_metadata.txt"),
                                os.path.join(self.clad_folder_path, "taps.csv")]
        missing = [p for p in extracted_file_paths if not os.path.isfile(p)]

        if not missing and force_clad_extraction == False:
            self.shape_brep_file=os.path.join(self.clad_folder_path, "shape.brep").replace("//",'/')
            self.unscaled_mesh_stl_file = os.path.join(self.clad_folder_path, "mesh.stl").replace("//",'/')
            self.metadata_file = os.path.join(self.clad_folder_path, "model_metadata.txt").replace("//",'/')
            self.unscaled_taps_csv_file = os.path.join(self.clad_folder_path, "taps.csv").replace("//",'/')

            print("Files already exist... Not Extracting CLAD Data")
            return
        else:

            # Create the output folder if it doesn't exist (it does, but do it anyway)
            os.makedirs(self.clad_folder_path, exist_ok=True)
            ...
            try:
                # Read the XMML file
                with open(self.clad_file_path, "r", encoding="utf-8") as file:
                    xmml_content = file.read()
            except Exception as e:
                raise RuntimeError(f"The CLAD file was unreadable -- address issues with CLAD file: {e}") from e
            
            try:
                # Parse the XML content
                root = ET.fromstring(xmml_content)
                
                # Extract the encoded shape data from the <shape> tag
                shape_tag = root.find(".//shape")  # Adjust the XPath if necessary
                
                # Extract metadata from the <model> tag
                model_tag = root.find(".//model")
                metadata = {}
                if model_tag is not None:
                    metadata = model_tag.attrib
                ...
                if shape_tag is not None:
                    encoded_data = shape_tag.text.strip()
                    
                    # Decode Base64
                    compressed_data = base64.b64decode(encoded_data)

                    # Decompress using zlib
                    mesh_data = zlib.decompress(compressed_data)

                    # Save to a BREP file
                    with open(self.shape_brep_file, "wb") as f:
                        f.write(mesh_data)
                    
                    
                    
                    import time
                    time.sleep(0.1)  # Wait 100ms after writing before reading
                    # Load the BREP file using FreeCAD
                    shape = Part.Shape()
                    try:
                        shape.read(self.shape_brep_file)
                    except Exception as e:
                        print("Shape read failed:", e)

                    # shape.exportStl(self.unscaled_mesh_stl_file) # tolerance=0.01, angular_tolerance=0.2)

                    # Create a mesh from the shape with custom resolution
                    mesh = MeshPart.meshFromShape(
                        Shape=shape,
                        LinearDeflection=0.2, # 0.1
                        AngularDeflection=0.523599, # 0.523599,   # Custom mesh size 
                        Relative=False
                    )
                    mesh.write(self.unscaled_mesh_stl_file)

                else:
                    pass
                
                # Extract taps data
                taps_tag = root.find(".//taps")
                if taps_tag is not None:
                    taps_data = []
                    for tap in taps_tag.findall("tap"):
                        label = tap.get("label", "")
                        ...
                        taps_data.append([label, tap_type, direction, x, y, z, Nx, Ny, Nz])
                    
                    # Save taps data to CSV file
                    with open(self.unscaled_taps_csv_file, "w", newline="") as csvfile:
                        csv_writer = csv.writer(csvfile)
                        csv_writer.writerow(["Label", "Type", "Direction", "x", "y", "z", "nx", "ny", "nz"])
                        csv_writer.writerows(taps_data)
                    
                else:
                    print("Error: <taps> tag not found in the file.")
            except Exception as e:
                raise RuntimeError(f"The CLAD file cannot be parsed -- please fix CLAD file errors: {e}") from e
    ...
    def get_scale_factor(self):
        ...
        try:
            data = {}
            with open(self.metadata_file, "r") as f:  # Replace with your file name
                for line in f:
                    line = line.strip()  # Remove leading/trailing spaces
                    if ":" in line:  # Ensure it's a valid label:value pair
                        label, value = line.split(":", 1)  # Split only on the first ':'
                        data[label.strip()] = value.strip()  # Store in a dictionary
            modelratio=float(data['modelRatio'])
            conversionDict={'Millimeter':1000,
                        'Centimeter':100,
                        'Meter':1} # need to convert unit string to a numerical value
            scalefactor = modelratio / conversionDict[data['modelUnits']]

        except Exception as e:
            raise RuntimeError(f" Problem with the meta data file, unable to extract scale factor: {e}") from e

        return scalefactor
    
    def get_scaled_mesh_and_taps(self):

        # Load in the unscaled data
        self.unscaled_mesh = o3d.io.read_triangle_mesh(self.unscaled_mesh_stl_file)
        self.unscaled_mesh.compute_vertex_normals()
        self.unscaled_taps = pd.read_csv(self.unscaled_taps_csv_file)

        # IF the scaled data already exists, just laod it in
        scaled_file_paths = [os.path.join(self.clad_folder_path, "taps_scaled.csv"),
                        os.path.join(self.clad_folder_path, "mesh_scaled.stl")]
        missing = [p for p in scaled_file_paths if not os.path.isfile(p)]

        if not missing:
            self.scaled_mesh = o3d.io.read_triangle_mesh(os.path.join(self.clad_folder_path, "mesh_scaled.stl"))
            self.scaled_mesh.compute_vertex_normals()
            self.scaled_taps = pd.read_csv(os.path.join(self.clad_folder_path, "taps_scaled.csv"))

        else:
            # compute the scaled tap positions
            scaledTapPositions = self.unscaled_taps[['x','y','z']].values *self.scale_factor

            #Scale the mesh
            self.scaled_mesh = o3d.geometry.TriangleMesh()
            self.scaled_mesh.triangles = self.unscaled_mesh.triangles
            self.scaled_mesh.vertices = o3d.utility.Vector3dVector(np.asarray(self.unscaled_mesh.vertices)*self.scale_factor)
            self.scaled_mesh.compute_vertex_normals()


            #Make a new dataframe with the scaled tap locations
            self.scaled_taps = self.unscaled_taps.copy()
            self.scaled_taps[['x','y','z']] = scaledTapPositions


            # Write out the scaled files
            self.scaled_taps.to_csv(self.clad_folder_path+"taps_scaled.csv",index = False)
            _=o3d.io.write_triangle_mesh(self.clad_folder_path+"mesh_scaled.stl",self.scaled_mesh)
    
    def align_and_sample_mesh_lattice(self):

        if os.path.isfile(self.clad_folder_path + f'mesh_pointcloud_spacing'+str(self.mesh_pcd_spacing).replace('.','d')+f'_multiplier{self.mesh_pcd_generation_multiplier}.pcd') and self.force_total_pcd_recompute == False:
            print('Mesh point cloud already saved, loading..')
            lattice_mesh_pcd = o3d.io.read_point_cloud(self.clad_folder_path + f'mesh_pointcloud_spacing'+str(self.mesh_pcd_spacing).replace('.','d')+f'_multiplier{self.mesh_pcd_generation_multiplier}.pcd')
            ...
            if not os.path.isfile(self.clad_folder_path + f'dense_mesh_pointcloud_spacing'+str(self.mesh_pcd_spacing).replace('.','d')+f'_multiplier{self.mesh_pcd_generation_multiplier}.pcd'):
                ...
                #o3d.io.write_point_cloud(self.clad_folder_path + f'dense_mesh_pointcloud_spacing'+str(self.mesh_pcd_spacing).replace('.','d')+f'_multiplier{self.mesh_pcd_generation_multiplier}.pcd', dense_mesh_pcd)
            # If it has, load it in. Save it as an attribute because it will be useful later for checking proximity to mesh.
            else:
                dense_mesh_pcd = o3d.io.read_point_cloud(self.clad_folder_path + f'dense_mesh_pointcloud_spacing'+str(self.mesh_pcd_spacing).replace('.','d')+f'_multiplier{self.mesh_pcd_generation_multiplier}.pcd')
                self.dense_mesh_pcd = dense_mesh_pcd
            ...
            o3d.io.write_point_cloud(self.clad_folder_path + f'mesh_pointcloud_spacing'+str(self.mesh_pcd_spacing).replace('.','d')+f'_multiplier{self.mesh_pcd_generation_multiplier}.pcd', lattice_mesh_pcd)
    ...
    def align_edge_points_to_lattice_excluding_endpoints(self):
        ...
        if os.path.isfile(self.clad_folder_path + f'mesh_edge_pointcloud_spacing'+str(self.mesh_pcd_spacing).replace('.','d')+f'_multiplier{self.mesh_pcd_generation_multiplier}.pcd') and self.force_total_pcd_recompute == False:
            print('Mesh edge point cloud already saved, loading..')
            aligned_edge_pcd = o3d.io.read_point_cloud(self.clad_folder_path + f'mesh_edge_pointcloud_spacing'+str(self.mesh_pcd_spacing).replace('.','d')+f'_multiplier{self.mesh_pcd_generation_multiplier}.pcd')
            ...
            mesh_vertices = self.scaled_mesh.vertices
            mesh = self.scaled_mesh
            mesh.compute_triangle_normals()
            mesh.remove_duplicated_vertices()
            mesh.remove_degenerate_triangles()
            ...
            o3d.io.write_point_cloud(self.clad_folder_path + f'mesh_edge_pointcloud_spacing'+str(self.mesh_pcd_spacing).replace('.','d')+f'_multiplier{self.mesh_pcd_generation_multiplier}.pcd',aligned_edge_pcd)
        return aligned_edge_pcd
    
    def get_total_pcd(self):

        if os.path.isfile(self.clad_folder_path + f'total_pcd_spacing'+str(self.mesh_pcd_spacing).replace('.','d')+f'_multiplier{self.mesh_pcd_generation_multiplier}.pcd') and self.force_total_pcd_recompute == False:
            total_pcd = o3d.io.read_point_cloud(self.clad_folder_path + f'total_pcd_spacing'+str(self.mesh_pcd_spacing).replace('.','d')+f'_multiplier{self.mesh_pcd_generation_multiplier}.pcd')
        else:
            ...
            if self.mesh_pcd is None or self.force_total_pcd_recompute == True:
                self.mesh_pcd = self.align_and_sample_mesh_lattice()
            if self.mesh_edge_pcd is None or self.force_total_pcd_recompute == True:
                self.mesh_edge_pcd = self.align_edge_points_to_lattice_excluding_endpoints()

            # We need the trimesh mesh for this
            try:
                self.scaled_mesh_trimesh = trimesh.load(os.path.join(self.clad_folder_path,"mesh_scaled.stl"))
                self.scaled_mesh_trimesh.fix_normals()
            except Exception as e:
                    raise RuntimeError(f"Could not load the scaled mesh as trimesh, run Building.get_scaled_mesh_and_taps: {e}") from e

            mesh_tm = self.scaled_mesh_trimesh
            ...
            o3d.io.write_point_cloud(self.clad_folder_path + f'total_pcd_spacing'+str(self.mesh_pcd_spacing).replace('.','d')+f'_multiplier{self.mesh_pcd_generation_multiplier}.pcd', total_pcd)
...
class ZLevel:
    ...
    def __init__(self, z_level, total_pcd, concave_hull_z_eps, concave_hull_length_threshold,concave_hull_concavity, sharp_angle_threshold,
                  nzFiltercutoff,corner_angle_threshold, use_average_corners, N_average_corner_points, vector_length_threshold, vector_width_threshold, major_cutoff_multiplier,
        ...
        self.use_average_corners = use_average_corners
        self.N_average_corner_points = N_average_corner_points
    ...
    def get_major_corners_and_directions(self):
        ...
        use_average_corners = self.use_average_corners
        N_average_corner_points = self.N_average_corner_points
        ...
        if len(corner_indices)>0:
            if use_average_corners == False:
                ...
                direction_vectors = np.array(self.get_avg_corner_vectors(concaveRing_nzFiltered[0], corner_indices,Navg=N_average_corner_points))
            else:
                raise ValueError("Set use_average_corners to True or False")
            all_ABs = direction_vectors[:,0]
            all_CBs = direction_vectors[:,1]
...
#building=Building(r"C:\\Users\\liam_\\Documents\\EngSims\\Phase 2\\ProductionTool\\TestToolForEngineers\\TestFiles\\cladTestFolder1\\1.clad")

#firstlevel = ZLevel(buildingObject.facade_z_levels[3]-1, buildingObject.total_pcd, buildingObject.facade_epsilon, buildingObject.facade_concave_hull_length_threshold, buildingObject.facade_hull_concavity, buildingObject.facade_corner_angle_threshold,
#                    buildingObject.facade_nzFiltercutoff,buildingObject.facade_corner_angle_threshold, buildingObject.facade_use_average_corners,buildingObject.facade_N_average_corner_points, buildingObject.roof_vector_length_threshold, buildingObject.roof_vector_width_threshold, buildingObject.facade_major_cutoff_multiplier,
#                    buildingObject.facade_horizontal_spacing, buildingObject.facade_anchor_distance, buildingObject.facade_double_tap_dist, double_tap = False)



#firstroof = Roof(buildingObject.roof_clusters[3],buildingObject.total_pcd,
#                 buildingObject.roof_concave_hull_length_threshold, buildingObject.roof_concave_hull_concavity,
#                    buildingObject.roof_corner_angle_threshold, buildingObject.roof_use_average_corners, buildingObject.roof_N_average_corner_points,
#                    buildingObject.roof_vector_length_threshold, buildingObject.roof_vector_width_threshold, buildingObject.roof_major_cutoff_multiplier,
#                    buildingObject.roof_horizontal_spacing, buildingObject.roof_anchor_distance, buildingObject.roof_double_tap_dist, buildingObject.roof_double_tap_list, buildingObject.roof_max_distance_from_model, buildingObject.roof_edge_maximum_filtering_distance,
#                    buildingObject.roof_distance_to_concave_hull_filtration,
#                    buildingObject.roof_center_upperdist, buildingObject.roof_center_mindist, buildingObject.roof_center_min_spacing_2,buildingObject.roof_center_max_iter, buildingObject.roof_polyline_neighbour_radius)
...
#print(firstlevel.taps)

#o3d.visualization.draw_geometries([building.scaled_mesh,building.PredictedTapPCD])